...
METaL is our protocol-level interface client allowing specified users to create and manage events, tickets and links on the Aventus Protocol. It provides a simple front end reference interface for users not willing to directly integrate with our API layer.
...
Before tickets are written to the blockchain (either via Modest or directly) they exist in a preliminary stage as a hyperlink created by METaL to be loaded (or claimed) by a ticket wallet. Here we'll define a ticket as generated by METaL.
...
It is important to note that the metadata of a ticket is not limited to what is outlined above, this is only what our reference client METaL is utilising. Just ensure whatever client you build needs to be able to understand the example JSON object structure.
