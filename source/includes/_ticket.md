...
On the Aventus Protocol - a ticket consists of specific pieces of data. Unique data, and metadata. Firstly we'll explain what is the content of a generated ticket link from METaL, then explain the contains of a Ticket used by the scaling solution.

<aside class="notice">
For the current purposes of what we're doing right now, we have two kinds of ticket. A ticket, and a merkle leaf (one that is using our scaling solution, Modest) ticket. Currently, all tickets that we will produce are merkle leaf tickets.
</aside>

## What's in a Ticket Link?
...
In terms of what a ticket looks like as a JSON object, see right (or below, if on mobile).
...
It is important to note that the Protocol has no opinion on what is inside of a ticket, it is up to clients to enforce and parse what a ticket really contains. With that in mind, it is possible to understand what extra pieces of metadata you may want to include inside of a ticket. It is not limited to what is above, but remember whatever client you build needs to be able to understand the JSON object structure above.
...
To the right is what the Ticket Wallet sends to the nessessary lambda.
